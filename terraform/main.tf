...
# ============================================================
# IRSA (IAM Roles for Service Accounts) - LabRole Integration
# Todos os microserviços (fiapx, fiapx-ms-processing) usarão a mesma LabRole
# ============================================================

# 1. TLS Certificate para OIDC Provider
...
resource "aws_iam_openid_connect_provider" "eks" {
  ...
  tags = { Name = "fiapx-irsa-provider" }
}

# 3. Locals para construir OIDC ARN e URL
locals {
  oidc_provider_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${replace(aws_eks_cluster.fiapx.identity[0].oidc[0].issuer, "https://", "")}"
  oidc_provider_url = replace(aws_eks_cluster.fiapx.identity[0].oidc[0].issuer, "https://", "")
}

# 4. Data source para referenciar LabRole existente
data "aws_iam_role" "lab_role" {
  name = "LabRole"
}

# 5. Adicionar Trust Policy à LabRole para permitir IRSA
# Permite que ServiceAccounts do EKS assumam LabRole via WebIdentity
resource "aws_iam_role_policy" "lab_role_irsa_trust" {
  name = "lab-role-irsa-trust-policy"
  ...
  lifecycle {
    ignore_changes = [policy]  # Evitar conflitos com políticas externas
  }
}

# Output para referência nos K8s manifests
output "irsa_lab_role_arn" {
  description = "ARN da LabRole para usar em ServiceAccount annotations"
